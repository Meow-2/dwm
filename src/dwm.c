...
buttonpress(XEvent *e)
{
    ...
    if (ev->window == selmon->barwin || (!c && selmon->showbar && (topbar ? ev->y <= selmon->wy : ev->y >= selmon->wy + selmon->wh))) { // 点击在bar上
        ...
        if (i < LENGTH(tags)) {
            ...
            x = selmon->ww - status_w - 2 * sp - (selmon == systraytomon(selmon) ? (system_w ? system_w + systraypinning + 2 : 0) : 0);
            click = ClkStatusText;
...
drawstatusbar(Monitor *m, int bh, char* stext) {
    int status_w = 0, i, j,w, x, len, system_w = 0;
    short isCode = 0;
    char *text;
    char *p;
    char buf8[8], buf5[5];
...
updatestatus(void)
{
    ...
    if (!gettextprop(root, XA_WM_NAME, stext, sizeof(stext)))
        strcpy(stext, "^c#2D1B46^^b#335566^    别来无恙  ^d^"); // 默认的状态栏文本
    for (m = mons; m; m = m->next)
        drawbar(m);
    updatesystray();
}
